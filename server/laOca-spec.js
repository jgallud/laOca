...
describe("El juego de la Oca...",function(){
	...
	describe("En cuanto a la inicialización",function(){
		beforeEach(function(){
			var control =new modulo.Control();
			var id;
			colores=["rojo","azul"];
			this.id=control.agregarPartida((new modulo.LaOcaFactory(colores)).crearJuego());
			this.partida=control.partidas[this.id];
		});

		it("...la variable juego debe estar definida",function(){
			expect(this.partida).toBeDefined();
		});

		it("...el juego debe tener un tablero",function(){
			expect(this.partida.tablero).toBeDefined();
		});

		it("...el tablero debe tener 64 elementos",function(){
			expect(this.partida.tablero.casillas.length).toEqual(64);
		});

		it("...el juego tiene una coleccion de fichas", function(){
			expect(this.partida.coleccionFichas).toBeDefined();
		});

		it("...la coleccion de fichas debe tener 2 fichas",function(){
			expect(this.partida.coleccionFichas.length).toEqual(2);
		});

		it("...inicialmente, todas las fichas están libres",function(){
			var todasLibres=_.find(this.partida.coleccionFichas,function(f){
				return f.libre==false});
			expect(this.todasLibres).toBeUndefined();
		});

		it("...el juego tiene una coleccion de jugadores",function(){
			expect(this.partida.coleccionJugadores).toBeDefined();
		});

		it("...el juego permite crear un jugador llamado Pepe",function(){
			this.jugador=new modulo.Jugador("Pepe",this.partida);
		...
		it("...el juego permite asignar una ficha libre a Pepe",function(){
			...
			expect(this.partida.coleccionJugadores[0].nombre).toMatch("Pepe");
	...
	describe("Comprobar el tablero...",function(){
		beforeEach(function(){
			colores=["rojo","azul"];		
			this.partida = (new modulo.LaOcaFactory(colores)).crearJuego();
		});

		it("...las casillas 6 y 12 tienen Puente",function(){
			expect(this.partida.tablero.casillas[6].tema.titulo).toEqual("Puente");
			expect(this.partida.tablero.casillas[12].tema.titulo).toEqual("Puente");
		});

		it("...la casilla 19 tiene una Posada",function(){
			expect(this.partida.tablero.casillas[19].tema.titulo).toEqual("Posada");			
		});

		it("...las casillas 26 y 53 tiene Dados",function(){
			expect(this.partida.tablero.casillas[26].tema.titulo).toEqual("Dados");
			expect(this.partida.tablero.casillas[53].tema.titulo).toEqual("Dados");
		});

		it("...la casilla 42 tiene Laberinto",function(){
			expect(this.partida.tablero.casillas[42].tema.titulo).toEqual("Laberinto");
		});

		it("...la casilla 52 tiene Cárcel",function(){
			expect(this.partida.tablero.casillas[52].tema.titulo).toEqual("Carcel");
		});

		it("...la casilla 58 tiene Calavera",function(){
			expect(this.partida.tablero.casillas[58].tema.titulo).toEqual("Calavera");
		});

		it("...las casillas 5,9,14,18,23,27,32,36,41,45,50,54 y 59 tienen Oca",function(){
			expect(this.partida.tablero.casillas[5].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[9].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[14].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[18].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[23].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[27].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[32].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[36].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[41].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[45].tema.titulo).toEqual("Oca");
			expect(this.partida.tablero.casillas[50].tema.titulo).toEqual("Oca");			
			expect(this.partida.tablero.casillas[54].tema.titulo).toEqual("Oca");		
			expect(this.partida.tablero.casillas[59].tema.titulo).toEqual("Oca");	
		})
	});
	...
	describe("Comprobamos el funcionamiento de los turnos con tres jugadores",function(){
		...
		beforeEach(function(){
			...
			this.partida = (new modulo.LaOcaFactory(colores)).crearJuego();
			this.ju1=new modulo.Jugador("Pepe",this.partida);
			this.ju1.asignarFicha();
			this.ju2=new modulo.Jugador("Luis",this.partida);
			this.ju2.asignarFicha();
			this.ju3=new modulo.Jugador("Lola",this.partida);
			this.ju3.asignarFicha();
		});		

		it("El turno lo tiene jug1, cae en una casilla normal y pasa el turno a jug2",function(){
			//this.partida.setTurno(this.ju1);
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(1);
		...
		it("El turno lo tiene jug2, cae en una casilla normal y pasa el turno a jug3",function(){
			//this.partida.setTurno(this.ju2);
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(1);
			this.ju1.ficha.mover(1);
			expect(this.ju1.turno.nombre).toMatch("NoMeToca");
			expect(this.ju3.turno.nombre).toMatch("NoMeToca");			
			expect(this.ju2.turno.nombre).toMatch("MeToca");			
			this.ju2.ficha.casilla=this.partida.tablero.getCasilla(1);
		...
		it("El turno lo tiene jug3, cae en una casilla normal y pasa el turno a jug1",function(){
			//this.partida.setTurno(this.ju3);
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(1);
			this.ju1.ficha.mover(1);
			//expect(this.ju1.turno.nombre).toMatch("NoMeToca");
			//expect(this.ju3.turno.nombre).toMatch("NoMeToca");			
			...
			this.ju2.ficha.casilla=this.partida.tablero.getCasilla(1);
			this.ju2.ficha.mover(1);
			this.ju3.ficha.casilla=this.partida.tablero.getCasilla(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 14 y pasa a la 18 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(13);
				//this.partida.fase.lanzar(this.ju1.ficha, 1);
				this.ju1.ficha.mover(1);
		...
		it("jug1 cae en casilla 18 y pasa a la 23 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(17);
				this.ju1.ficha.mover(1);
		...
		it("jug1 cae en casilla 23 y pasa a la 27 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(22);
				this.ju1.ficha.mover(1);
		...
		it("jug1 cae en casilla 27 y pasa a la 32 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(25);
...
<<<<<<< HEAD
		it("jug1 cae en casilla 59 y pasa a la 63 y mantiene el turno",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(58);
=======
...
>>>>>>> 0f2abb40384488379e44fb25f26f8a0fe83aef1a
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("La fase es Jugar, el turno lo tiene jug1",function(){
			expect(this.partida.fase.nombre).toEqual("Jugar");
			expect(this.ju1.turno.nombre).toMatch("MeToca");
		});

		it("jug1 cae en casilla 6 y pasa a la 12 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(5);
				this.ju1.ficha.mover(1);
		...
		it("jug1 cae en casilla 12 y pasa a la 6 y mantiene el turno",function(){
				this.ju1.ficha.casilla=this.partida.tablero.getCasilla(11);
				this.ju1.ficha.mover(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 19 pierde dos turnos, el turno pasa a jug2",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(18);
			this.ju1.ficha.mover(1);
		...
		it("jug1 está en la Posada y pierde dos turnos",function(){
			...
			expect(this.ju1.turno.nombre).toMatch("NoMeToca");
			expect(this.ju1.estado.nombre).toEqual("Pierde2Turnos");
			//this.partida.turno.lanzar(this.ju2.ficha,1);
			//this.partida.cambiarTurno(this.ju2);
			this.ju2.ficha.mover(1);	
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 53 y pasa a la 26 y mantiene el turno",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(52);
			this.ju1.ficha.mover(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 31, pierde el turno y espera a que otro jugador caiga",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(30);
			this.ju1.ficha.mover(1);
		...
		it("jug1 está en el pozo y jug2 cae y saca a jug1",function(){
			...
			this.ju1.ficha.mover(1);
			expect(this.ju1.ficha.casilla.posicion).toEqual(31);
			this.ju2.ficha.casilla=this.partida.tablero.getCasilla(30);
			this.ju2.ficha.mover(1);
			expect(this.ju1.estado.nombre).toEqual("Vivo");
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 42 pierde dos turnos",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(41);
			this.ju1.ficha.mover(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 52 pierde tres turnos",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(51);
			this.ju1.ficha.mover(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 58 pierde y vuelve a la 1",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(57);
			this.ju1.ficha.mover(1);
	...
	//		this.partida.setTurno(this.ju1);
		...
		it("jug1 cae en casilla 63 y gana, el juego termina",function(){
			this.ju1.ficha.casilla=this.partida.tablero.getCasilla(62);
			this.ju1.ficha.mover(1);
			expect(this.ju1.ficha.casilla.posicion).toEqual(63);
			expect(this.partida.fase.nombre).toEqual("Fin");
		});					
	})
})